				...
				<div id="mypage">
					...
					<div class="viewDivMt">
						...
						<div class="viewContents">
							...
							<br>
							<img src="/images/${bdto.bfile }" alt="" style="width:100%" />
						</div>
					</div>
					<!-- 이전다음글 -->
					<div class="pnDiv web">
					...
					<script type="text/javascript">
						...
						function commentBtn(){
							...
							if($(".replyType").val().length < 2 ){
								alert("두글자 이상 입력하셔야 등록할 수 있습니다 ");
								return false;
							}
							//alert($(".replyType").val()+$(".replynum").val());
							alert("댓글 저장합니다");
							// ajax
							$.ajax({
...
//									dataHtml += '<li class="name">'+data.id+'<span>&nbsp&nbsp[ '+	moment(data.cdate).format() +']</span></li>';
									...
									//dataHtml += '<a onclick="updateBtn('+data.cno+')" class="rebtn">수정</a>&nbsp';
						...
						function updateBtn(cno,id,cdate,ccontent){
							...
							if(confirm("댓글을 수정하시겠습니까 ?")){
								...
								dataHtml += '&nbsp;&nbsp;&nbsp;&nbsp;비밀번호&nbsp;&nbsp;<input type="password" class="replynum" id="updatePw" /></li>';
								dataHtml += '<li class="txt">';
								dataHtml += '<textarea class="replyType" id="updateTxt">'+ccontent+'</textarea></li>';
								dataHtml += '<li class="btn">';
						...
						function updateSave(cno){
							//alert(cno + $("#updateTxt").val() +$("#updatePw").val());
							
							if(confirm("댓글 수정을 저장하시겠습니까 ?")){
							
								$.ajax({
									url:"/board/commentUpdateSave",
									type:"post",
									data:{"cno": cno, "cpw":$("#updatePw").val(), "ccontent": $("#updateTxt").val() },
									success:function(data){
										alert("댓글 수정 성공");
